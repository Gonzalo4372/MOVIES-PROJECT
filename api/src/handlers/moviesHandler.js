...
const getAllMoviesHandler = async (req, res) => {
 ...
 try{
    ...
    const results = title ? await searchMovieByTitle(title): await getAllMovies();
...
const createMovieHandler = async (req, res) => {
   try {
    ...
    const {title, image, overview, release_date, genres, popularity} = req.body;
    ...
    res.status(200).send(newMovie);
      ...
      return res.status(404).send({error: "Missing required field"})
    }
   }
